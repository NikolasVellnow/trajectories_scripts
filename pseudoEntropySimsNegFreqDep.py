...
cList = ['0.25', '0.5', '0.75', '0.99']

domCoeffList = ['0.5']


timeStr = time.strftime("%Y%m%d_%H%M%S")
modelStr = 'pseudoEntropySimsNegFreqDep_sel_n400'
...
print("Total number of runs to be executed: " +
      str(numRuns*len(rList)*len(cList)*len(domCoeffList)))
...
for currentDomCoeff in domCoeffList:
    ...
    for currentC in cList:
        print("Proportionality constant " + currentC)
    ...
    # loop through list of recombination rates
        for currentR in rList:
            ...
            simulationFileName = args.input_file.split(
                '.')[0] + '_' + currentDomCoeff + '_' + currentR + '_' + currentC + '.' + args.input_file.split('.')[1]
            ...
            for i in range(numRuns):
                ...
                slimFile.write('Dominance coefficient: ' +
                               currentDomCoeff + '\n')
                slimFile.write('Proportionality constant: ' + currentC + '\n')
                ...
                for line in output.split('\n'):
                    ...
                    # check if mutation with desired frequency has been found
                    if (isMutNotFound(line)):
                        break
                        ...
                        for indx in range(len(currentIds)):
                            currentFreqDict.setdefault(
                                currentIds[indx], currentFreqs[indx])
                ...
                if (mutNotFound == True):
                    ...
                    for id in t_0_Dict.keys():  # loop through mutation ids (str)
                        for gen in generationDict.keys():   # loop through generation numbers (int)
                            ...
                            if (id not in currentGenDict.keys()):
                                # insert frequencies of 0.0 for lost mutations
                                generationDict.get(gen).setdefault(
                                    id, Decimal(0.0))
                            for k in list(currentGenDict.keys()):
                                if (k not in t_0_Dict.keys()):
                                    # remove mutation that was not in population at the start of sampling period
                                    currentGenDict.pop(k)
                    ...
                    for id in t_0_Dict.keys():      # loops through mutation ids found in population
                        ...
                        for gen in generationDict.keys():       # loops through sampled generations
                            ...
                            currentCtr = '{x:.4f}'.format(x=currentFreq) + '\t'
                            freqsFile.write(currentCtr)
                        freqsFile.write('\n')
